...
{
    public class ThietLapContext : DbContext
    {
        ...
        public ThietLapContext()
        {
        }

        public ThietLapContext(DbContextOptions<ThietLapContext> options) : base(options)
        {
        }
        ...
        public DbSet<BangPhatDiMuon> PhatDiMuons { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Luong>()
               .HasOne(l => l.BangPhatDiMuon)
               .WithMany(nv => nv.Luong)
               .HasForeignKey(l => l.MaNV);
        ...
        public async Task<DataTable> GetLuongAsync(string maNV = null, string thangLinhLuong = null)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("GetLuong", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    if (!string.IsNullOrEmpty(maNV))
                    {
                        cmd.Parameters.Add(new SqlParameter("@maNV", SqlDbType.VarChar, 10) { Value = maNV });
                    }
                    else
                    {
                        cmd.Parameters.Add(new SqlParameter("@maNV", SqlDbType.VarChar, 10) { Value = DBNull.Value });
                    }

                    if (!string.IsNullOrEmpty(thangLinhLuong))
                    {
                        cmd.Parameters.Add(new SqlParameter("@thangLinhLuong", SqlDbType.Char, 7) { Value = thangLinhLuong });
                    }
                    else
                    {
                        cmd.Parameters.Add(new SqlParameter("@thangLinhLuong", SqlDbType.Char, 7) { Value = DBNull.Value });
                    }
